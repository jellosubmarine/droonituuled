...
Visuals::Visuals(ros::NodeHandle &nh_) : it_(nh_)
{
  ...
  if (nh_.param("threshold/saturation", CAM_SATURATION_THRESH, 50))
  {
    ROS_INFO("Got param: CAM_SATURATION_THRESH %i", CAM_SATURATION_THRESH);
  }
  else
  {
...
void Visuals::process()
{
  ...
  if (W > 0)
  {
    ...
    {
      ROS_INFO_STREAM(
          "Dropped point [" << centroids[max_dist_i].x + CAM_FRAME_OFFSET_X << ", " << CAM_FRAME_HEIGHT - centroids[max_dist_i].y - CAM_FRAME_OFFSET_Y << "]");
    ...
    { // To calculate cov, we need vectors with components
