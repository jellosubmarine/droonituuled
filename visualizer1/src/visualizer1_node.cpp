...
frame = cv_bridge::toCvShare(mconst sensor_msgs::ImageConstPtr& ,
"bgr8")->image; // get a new frame from camera
...
dilate(canny_output, canny_output, Mat(), Point(-1, -1), 2, 1, 1); //
Attempt to get rid of noise
erode(canny_output, canny_output, Mat(), Point(-1, -1), 2, 1, 1);
findContours(canny_output, contours, hierarchy, RETR_TREE,
CHAIN_APPROX_SIMPLE, Point(0, 0) ); // Get contours
...
for( size_t i = 0; i < mu.size(); i++ ) // Based on moments calculate
centroids
{
...
sum_x = accumulate(avg_x.begin(), avg_x.end(), 0); // Sum all centroids x
values
if(avg_x.size() == 0){ X = -1;}
else {X = (sum_x/avg_x.size());} // Get average x coordinate.

sum_y = accumulate(avg_y.begin(), avg_y.end(), 0); // Sum all centroids y
values
if(avg_y.size() == 0){ Y = -1;}
else {Y = (sum_y/avg_y.size());} // Get average y coordinate.

vector<Point2f> XY(1);
XY[0] = Point2f( static_cast<float>(X) ,static_cast<float>(Y)); // For
visuals
...
duration = ( std::clock() - start ) / (double) CLOCKS_PER_SEC; // Get FPS
here
